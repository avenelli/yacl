...
logging.basicConfig(
    filename=os.path.join(DIRLOG,"Test_ZpAdd.log"),
...
ValFiles = glob.glob(os.path.join(DIRVAL,"Test_ZpAdd*.val"))

for f in ValFiles:
    # Parse values file
    ListStruct = ParseValuesFile(f,"Add")

    logging.info("ZPADD C = A+B - %s" % f)
    NbOK = 0
    Idx = 1
    # Launch Test : C = A+B
    for e in ListStruct:
        ...
        LaunchAdd = [0x01,ClassCom.INS_COMMAND,ClassCom.CMD_ZPADD,0x00,0x08]+AdrC+AdrA+AdrB+[e.LengthBytes+4,0x00]
        ClassCom.SERSend(LaunchAdd)
